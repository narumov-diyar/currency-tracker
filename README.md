...
**Тема.** Веб-сервис «Трекер курсов валют»: персональный список валютных и криптовалютных пар с отображением текущего курса, кросс-курса, истории изменений в виде графика, сравнения динамики двух активов пары и пользовательских оповещений о достижении заданного курса. Поддерживаются как криптовалютные активы, так и обычные (фиатные) валюты, что позволяет составлять пары любого сочетания — две криптовалюты, две фиатные валюты либо криптовалюта и фиатная валюта.

## Этап 2. Выработка требований

### Пользовательские сценарии

Сервер хранит и отдаёт цену каждого актива отдельно, приведённой к единой базе — USDT, а не цену каждой возможной пары. Кросс-курс пары вычисляется на клиенте делением одного полученного ряда на другой, благодаря чему любое сочетание активов поддерживается без отдельного хранения.
...
Ряды разной частоты совмещаются правилом переноса последнего известного значения: при построении пары значение более редкого ряда берётся по состоянию на момент времени более частого. Это же правило закрывает выходные и праздничные дни, когда котировки фиатных валют не обновляются.

Курсы фиатных валют приводятся к USDT по допущению USDT ≈ USD; погрешность приближения составляет около 0,1 % и для задач мониторинга признана несущественной.
...
- Смена периода перерисовывает график по данным хранилища без обращения к внешним источникам; отсутствие данных за период отображается как пустое состояние.
...
- Вкладка «Реальное время» доступна, если потоковый режим поддерживают оба актива пары; она отображает последние 15 минут с обновлением не чаще одного раза в секунду. Поступающие котировки в базу данных не записываются.

**US-3. Оповещения о достижении целевого курса.** Как пользователь, я хочу задавать пороговое значение курса актива, чтобы получать оповещение при его достижении без постоянного наблюдения за графиком.

Критерии приёмки:

- Правило содержит актив, направление («выше», «ниже») и целевое значение курса; пользователь просматривает, изменяет и удаляет собственные правила.
- Правило хранится в базе данных и действует независимо от наличия открытого сеанса пользователя.
- Проверка правил выполняется сервером при сохранении каждого нового снимка цены соответствующего актива, то есть с частотой, установленной для его типа; правило считается сработавшим при пересечении порога между двумя соседними снимками, а не при точном совпадении цены с пороговым значением.
...
**Хранение и понижение детализации.** Хранить минутные свечи бессрочно нецелесообразно: за пять лет это 78,8 млн строк, или около 11,4 ГБ. Вместо удаления устаревших данных применяется понижение детализации: фоновая задача раз в сутки объединяет свечи закрытых периодов в более крупные, где цена открытия берётся от первой свечи интервала, цена закрытия — от последней, максимум и минимум — как экстремумы, объём и оборот — как суммы. Исходные строки удаляются в той же транзакции, в которой записан агрегат, поэтому повторный запуск задачи безопасен.
...
read_traffic        ≈ 1,13 ГБ/сутки без сжатия
```
